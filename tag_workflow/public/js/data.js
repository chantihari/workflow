...
frappe.ui.form.ControlData = class ControlData extends frappe.ui.form.ControlInput {
	...
	bind_change_event() {
		const change_handler = (e) => {
			...
			if(e.type == "change" && (this.df.fieldtype == "Text" || this.df.fieldtype == "Data" || this.df.fieldtype == "Small Text") && this.doctype && !["phone_no", "accounts_payable_phone_number", "accounts_receivable_phone_number", "phone_number", "contact_number", "mobile_no"].includes(this.df.fieldname) && this.df.options!="Email"){
				if(/[+<>^?#/\\]/.test(value)){
					frappe.msgprint({
						title: __('Error'),
