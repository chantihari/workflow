...
frappe.ui.form.Layout = class Layout {
  ...
  make() {
    ...
    this.render();
    let history_length = frappe.route_history.length;
    if (this.doctype == "User") {
      if (
        history_length == 1 &&
        JSON.stringify(frappe.route_history[0]) !==
          JSON.stringify(["Form", "User", this.frm.doc.name])
      ) {
        window.location.reload();
      } else if (history_length > 1) {
        if (frappe.route_history[history_length - 2][1] == "Job Site") {
          return;
        } else if (
          JSON.stringify(frappe.route_history[history_length - 2]) !==
          JSON.stringify(["List", "User", "List"])
        ) {
          window.location.reload();
        }
      }
  ...
  setup_tabbed_layout() {
    if ($(window).width() > 1200) {
      ...
      ["Company", "Employee", "Lead", "User"].includes(this.frm.doctype)
    ) {
      $(`
      <div class="btn-group company_setting_list pt-4 pl-4 pb-0">
          <button class="btn btn-sm dropdown-toggle border" id="selected" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            Company Info
          </button>
          <div class="dropdown-menu dropdown-list form-tabs-list">
            <ul class="nav form-tabs" id="form-tabs1" role="tablist"> </ul>
          </div>
      </div>
      `).appendTo(this.page);
  ...
  make_tab(df) {
    ...
    this.tabs.push(tab);
    if ($(window).width() <= 1200) {
      $("#selected").text($("#form-tabs1>li")[0].innerText);
  ...
  set_dependant_property(condition, fieldname, property) {
    ...
    if (form_obj) {
      if (this.doc?.parent && this.doc.parentfield) {
  ...
  evaluate_depends_on_value(expression) {
    ...
    if (typeof expression === "boolean") {
      ...
      try {
        ...
        if (parent?.istable && expression.includes("is_submittable")) {
          out = true;
        }
      } catch (e) {
