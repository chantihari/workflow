...
frappe.ui.form.ControlLink = class ControlLink extends frappe.ui.form.ControlData{
	...
	new_doc() {
		...
		if (df?.get_route_options_for_new_doc) {
	...
	setup_awesomeplete() {
		...
		this.custom_awesomplete_filter?.(this.awesomplete);
	...
	set_custom_query(args) {
		...
		if(this.get_query || this.df.get_query) {
			...
			if($.isPlainObject(get_query)) {
				...
				let q = (get_query)(this.frm?.doc || this.doc, this.doctype, this.docname);
	...
	get_fetch_map() {
		...
		for (const key of ["*", this.df.parent]) {
			if (this.frm.fetch_dict[key]?.[this.df.fieldname]) {
