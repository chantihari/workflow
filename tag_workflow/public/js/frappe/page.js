...
frappe.ui.Page = Class.extend({
  ...
  setup_page: function () {
    ...
    if (
      ...
      this.hiring_filter = $(`
            ...
            <button class="btn btn-default btn-sm filter-button" onclick="hiring_click('Canceled');">
							<span class="button-label hidden-xs">Canceled Orders<span></span></span>
						</button>
    ...
    if (
      ...
      this.staffing_filter = $(`
                  ...
                  <button class="btn btn-default btn-sm filter-button" id='staff_filter_button6' title="Canceled" value='Canceled Order'>
											<span class="button-label hidden-xs">Canceled Orders<span></span></span>
									</button>
      ...
      if (
        ...
        this.item_filter = $(`
				<div class="page-form row" id="item_filter">
					<div class="filter-selector">
						<button class="btn btn-default btn-sm filter-button" onclick="filter_job_titles(true)" id='filter_selected_data';">
							<span class="button-label hidden-xs">My Job Titles<span></span></span>
						</button>
						<button class="btn btn-default btn-sm filter-button" onclick="filter_job_titles(false)" id='filter_all_data';">
							<span class="button-label hidden-xs">All Job Titles<span></span></span>
						</button>
					</div>
          <script>
            function filter_job_titles(val){
              $('.page-form').find('input').each(function () {
                if($(this).attr('data-fieldname')=='is_company'){
                  this.checked=val;
                  cur_list.refresh();
                }
              });
  ...
  set_view: function (name) {
    ...
    this.current_view?.toggle(false);
    this.current_view = this.views[name];
