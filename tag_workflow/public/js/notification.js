...
class NotificationsView extends BaseNotificationsView {
	...
	get_dropdown_item_html(field) {
		...
		if(message.includes("has submitted a timesheet on")){
			message_html=`<div title="${field.document_name}" onclick="set_local('${message.split(" for ")[message.split("for").length - 2]}')" class="message">
				<div>${message}</div>
				<div class="notification-timestamp text-muted">
					${timestamp}
				</div>
		...
		}else if(field.document_type=="Claim Order" && message.includes("has submitted a claim for")){
			message_html=`<div title="${field.document_name}" onclick="claim_navigation(this.title)" class="message">
				<div>${message}</div>
				<div class="notification-timestamp text-muted">
					${timestamp}
				</div>
	...
	render_notifications_dropdown() {
		...
		} else if (this.dropdown_items.length) {
...
function set_local(job_order_detail){
		localStorage.setItem("order", job_order_detail);
		window.location.href = "/app/timesheet-approval";
}

function claim_navigation(title){
	window.location.href ="/app/claim-order/view/list?job_order="+title;
}
