	...
	async setup_toolbar() {
		...
		this.page.add_button(
			...
			() => {
				...
				this.render_page('/printview?', false, current_view || '')
			},
			{ icon: 'full-page' }
		);
		...
		this.page.add_button(
			...
			() => {
				let current_view = localStorage.getItem('current_invoice_view');
				this.render_page('/api/method/frappe.utils.print_format.download_pdf?', false, current_view || '');
			},
			{ icon: 'small-file' }
		);
	}
	...
	render_page(method, printit = false, view='') {
		let w = window.open(
			frappe.urllib.get_full_url(
				...
				'&format=' +
				encodeURIComponent(view || this.selected_format()) +
				'&no_letterhead=' +
