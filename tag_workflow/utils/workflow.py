...
def bulk_workflow_approval(docnames, doctype, action):
	...
	for (idx, docname) in enumerate(docnames, 1):
		...
		try:
			...
			final(failed_transactions, successful_transactions, docname, message_dict)
...
def final(failed_transactions, successful_transactions, docname, message_dict):
    if not message_dict:
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		if "Successful" in title:
			msg = "Timesheet(s) has been added successfully"
			frappe.msgprint(msg, title=_("Submitted"), indicator=indicator, is_minimizable=True)


		else:
			workflowstatus(messages, doctype, indicator, msg)

def workflowstatus(messages, doctype, indicator, msg):
